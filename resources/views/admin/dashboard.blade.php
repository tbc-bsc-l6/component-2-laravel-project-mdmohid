    ...
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow rounded-lg p-6 mb-8">
            ...
            <form method="POST" action="{{ route('admin.modules.store') }}" class="flex gap-4">
                @csrf
                <input type="text" name="module" placeholder="Module name" required class="flex-1 border rounded px-4 py-2">
                <button type="submit" class="text-green-700 hover:text-green-900" hover:bg-indigo-700">Add</button>
            </form>
        </div>
                        ...
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <div class="flex gap-3">
                                ...
                                <form method="PATCH" action="{{ route('admin.modules.toggle', $module->id) }}">
                                    @csrf @method('PATCH')
                                    <button type="submit" class="text-sm px-3 py-1 rounded {{ $module->active ? 'bg-yellow-100' : 'bg-green-100' }}">
                ...
                @forelse ($users as $user)
                    <div class="px-6 py-4 flex items-center justify-between">
                        ...
                        <form method="POST" action="{{ route('admin.users.change-role', $user->id) }}" class="flex gap-2">
                            ...
                            <button type="submit" class="bg-indigo-600 text-white px-3 py-1 rounded text-sm">Change</button>
                        </form>
                    </div>
                @empty
